// static/app.js
// Babycam: WebRTC video+audio with low-latency audio (NOT A/V-synced), motion indicator,
// sound meter, PiSugar battery, wake lock. Audio starts only after a user tap (mobile).
...
// IMPORTANT: split streams so audio is NOT delayed to sync with video.
...
async function startWebRTC() {
  ...
  pc.ontrack = (ev) => {
    ...
    if (ev.track.kind === "video") {
      try { videoStream.addTrack(ev.track); } catch (_) {}
    }

    if (ev.track.kind === "audio") {
      try { audioStream.addTrack(ev.track); } catch (_) {}
...
function clearStreams() {
  for (const t of videoStream.getTracks()) {
    try { videoStream.removeTrack(t); t.stop(); } catch (_) {}
  }
  for (const t of audioStream.getTracks()) {
    try { audioStream.removeTrack(t); t.stop(); } catch (_) {}
  }
}
...
// -----------------------------
// Sound meter (audioStream) – start ONLY after unlock
// -----------------------------
let audioCtx = null;
...
async function unlockAudio() {
  ...
  try {
    ...
    // Start sound meter from audioStream (low latency)
    startSoundMeterFromMediaStream(audioStream);
  } catch (e) {
